...
usage="Usage: ksh /global/save/Yali.Mao/scripts/convert_adds.sh CIP|FIP PRB|SEV YYYYMMDDHH [levels]"
...
COMIN=/dcomdev/us007003/$day/wgrbbul/adds_$prod_lc

DATA=${DATA:-/ptmpp1/`whoami`/${prod_lc}AWC.$day}
mkdir -p $DATA
cd $DATA
...
#          $UTIL/wgrib2 $file |grep "$search1" | grep "$search2" | /$UTIL/wgrib2 -i $file -grib x.$lvl
           $UTIL/wgrib2 $file -match "$search1" -match "$search2" -grib x.$lvl                            
           cat x.$lvl >> ${output}.t${hh}z.f${fh2}
         done #fh
       else # CIP
 	   search1=":d=$day$hh:IC${field}:$lvl"
           $UTIL/wgrib2 $file -match "$search1" -grib x.$lvl
           cat x.$lvl >> ${output}.t${hh}z.f00
       fi
     done # lvl
  else
