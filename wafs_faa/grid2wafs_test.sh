...
cd /stmpp1/Geoffrey.Manikin/wafs
if [ $# -ne 2 ] ; then
...
WGRIB=${WGRIB:-/meso/save/Geoffrey.Manikin/wafs/wgrib}
SCRIPTS=${SCRIPTS:-/meso/save/Geoffrey.Manikin/wafs}       
COPYGB_PARM=${COPYGB_PARM:-/meso/save/Geoffrey.Manikin/wafs/merge_copygb.parm}
COPYGB_IOPT=${COPYGB_IOPT:-1}
TIMEX=${TIMEX:-/meso/save/Geoffrey.Manikin/bin/timex}
CDATE=$1
rmdays=${RMDAYS:-7}
cdatem7=`/nwprod/util/exec/ndate -$((24*rmdays)) $CDATE`
if [ ${#CDATE} -ne 10 ] ;then 
...
elif [ $prodsuite = gdas ] ; then qual=gdas1
...
>$wafs_grib
#proddir=/ptmp/Geoffrey.Manikin/hold3
proddir=/com/gfs/prod/$prodsuite.$pdy
...
for file in pgrbanl pgrbf06;do
	...
	if [ ! -s $prodfile ] ; then 
		...
		prodfile=$proddir/$qual.$cyc.$file
		cd $proddir 
		$SCRIPTS/hpss_gdas_faa.sh $pdy `echo $cyc|cut -c2-3`
		prodfile=$proddir/$qual.$cyc.$file
	fi
#	$TIMEX $WGRIB -v $prodfile >wgrib
...
#echo "break break 1"
/nwprod/util/exec/wgrib $wafs_grib.unsorted -PDS -s |sed 's/PDS=............\(..\)[0-9a-f]*:*/grid=\1h:/' | \
	sed -e s/grid=25h/grid=5h/ -e s/grid=26h/grid=6h/ -e s/grid=27h/grid=7h/ -e s/grid=28h/grid=8h/ \
		-e s/grid=29h/grid=1h/ -e s/grid=2bh/grid=3h/ -e s/grid=2ch/grid=4h/ -e s/grid=2ah/grid=2h/ |\
	sort -t: -k6,6ir -k4,4 -k5.1,5.4rn -k8,8  |  \
	sed -e s/grid=5h/grid=25h/ -e s/grid=6h/grid=26h/ -e s/grid=7h/grid=27h/ -e s/grid=8h/grid=28h/ \
		-e s/grid=1h/grid=29h/ -e s/grid=3h/grid=2bh/ -e s/grid=4h/grid=2ch/ -e s/grid=2h/grid=2ah/ |
	/nwprod/util/exec/wgrib $wafs_grib.unsorted -i -s -grib -o $wafs_grib
...
ftpdir="/home/people/emc/ftp/mmb/mmbpll/wafstest"
