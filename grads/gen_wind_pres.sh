...
! write out in binary format for graphic display
       ...
       integer, parameter :: nmd=${nmdcyc}, ns=7,nmd0=${nmd0}, ncyc=${ncyc}
       ...
       character*20, parameter :: obsreg=" $obsv1 $reg VL1L2", vnam=" $vnam"
...
!-------------------------------------------------
                ...
                var_f(n,i,j,m)=max(0.0d0,vsdb(6)-vsdb(1)**2-vsdb(2)**2)
                var_a(n,i,j,m)=max(0.0d0,vsdb(7)-vsdb(3)**2-vsdb(4)**2)
                cor(n,i,j,m)=(vsdb(5)-vsdb(1)*vsdb(3)-vsdb(2)*vsdb(4))/ &
                           sqrt(var_f(n,i,j,m)*var_a(n,i,j,m))
                mse(n,i,j,m)=(max(0.0d0,(vsdb(6)+vsdb(7)-2*vsdb(5))))
                rms(n,i,j,m)=sqrt(mse(n,i,j,m))
                bias(n,i,j,m)=sqrt(vsdb(6))-sqrt(vsdb(7))
                e_m(n,i,j,m)=((vsdb(1)-vsdb(3))**2+(vsdb(2)-vsdb(4))**2)
