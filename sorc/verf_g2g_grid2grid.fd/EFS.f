...
c      REAL rfcst(ngrid,Nmodel),rfanl(ngrid),rclim (ngrid,kb)
...
c      real biasdata(ngrid),ftemp(ngrid)
...
c     +          rfanl(i).le.rclim(i,2)) then
          ...
          write(*,*) 'Entire:',iar,ivr, rfcst(5000,:),rfanl(5000),
     +      rclim(5000,:)
...
cc
cc  Following is for output in Yuejian's format
cc 
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
