...
C         if(igribid.gt.10) jpds(14)=ff(nfcst)           !skip GFS grib
         ...
         write(*,*) 'yy,mm,dd,hh=',yy(nfcst),mm(nfcst),dd(nfcst) 
                                                                                                     
         jpds(8)=yy(nfcst)
         jpds(9)=mm(nfcst)
         jpds(10)=dd(nfcst)
         jpds(11)=hh(nfcst)   !cycle time
c         jpds(14)=ff(nfcst)   !forecast time (lead time)
...
c             write(*,*)  ' before    jpds=',jpds
c             write(*,*)  ' before    kpds=',jpds
c             write(*,*)  ' before    jgds=',jgds
             call getgb(grbunit, indxunit, ngrid, 0, jpds, jgds,
     &                      kf, k, kpds, kgds, lb, var, iret)
c             write(*,*) ' after     jpds=',jpds
c             write(*,*) ' after     kpds=',kpds
c             write(*,*) ' after     kgds=',kgds
