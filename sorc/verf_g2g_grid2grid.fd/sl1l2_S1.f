      subroutine SL1L2_S1(nvsdb,imodel,ist,numfcst,numvfyobs,numarea,
     +  numvarbl,numlevel,ngrid,levels,hasdata,fcstdata,obsvdata)

c    This program is to generate SL1L2+S1 vsdb records for all fcst times,
c    all variables, all levels, all requested sub-regions, then write them
c    into the vsdb file
c    Author: Binbin Zhou
c            March, 2005
c    Modification: add S1 to extend SL1L2 by Binbin Zhou
c            Jan, 2008            
     ...
     +      sumgrid,sumprod,ssqdata,ssqgrid,count,weigh
      ...
      !declared for S1 score
      REAL, allocatable, dimension(:,:) :: fcst2d, obsv2d,
     +                        dxf2d,dyf2d,dxo2d,dyo2d
      REAL, allocatable, dimension(:) :: dxf,dyf,dxo,dyo 
      real*8,  allocatable, dimension(:,:,:,:,:) :: sumMAE,
     +      sum1,sum2,sum3,sum4                                                      
...
C
     ...
     +  numvarbl,numlevel,ngrid
      ...
      write(*,*) 'numreg(iar)=',numreg
        ...
        allocate (dxf2d(imax(1),jmax(1)))
        allocate (dxo2d(imax(1),jmax(1)))
        allocate (dyf2d(imax(1),jmax(1)))
        allocate (dyo2d(imax(1),jmax(1)))
        allocate (dxf(ngrid))
        allocate (dxo(ngrid))
        allocate (dyf(ngrid))
        allocate (dyo(ngrid))

        allocate (sumMAE(numfcst,numvarbl,numlevel,numarea,numvfyobs))
        allocate (sum1(numfcst,numvarbl,numlevel,numarea,numvfyobs))
        allocate (sum2(numfcst,numvarbl,numlevel,numarea,numvfyobs))
        allocate (sum3(numfcst,numvarbl,numlevel,numarea,numvfyobs))
        allocate (sum4(numfcst,numvarbl,numlevel,numarea,numvfyobs))
...
c      end do
             ...
             do 50 ilv = 1, levels(ivr)
              if(nodata(ifh,ivr,ilv).eq.1) goto 50
               ...
               do j = 1, jmax(1)
                do i = 1, imax(1)
                 ij=(j-1)*imax(1) + i
                 fcst2d(i,j)=fcstdata(ifh,ivr,ilv,ij)
                 obsv2d(i,j)=obsvdata(ifh,ivr,ilv,ij)
                end do
               end do
               ...
               do j = 1, jmax(1)-1
                do i = 1, imax(1)-1
                 dxf2d(i,j)=fcst2d(i+1,j)-fcst2d(i,j)
                 dxo2d(i,j)=obsv2d(i+1,j)-obsv2d(i,j)
                 dyf2d(i,j)=fcst2d(i,j+1)-fcst2d(i,j)
                 dyo2d(i,j)=obsv2d(i,j+1)-obsv2d(i,j)
                end do
               end do
     ...
     +                    sumMAE(ifh,ivr,ilv,iar,iob) +
     +                    abs(fcstdata(ifh,ivr,ilv,i)-
     +                        obsvdata(ifh,ivr,ilv,i) )
     +                        *area_factor(i)

                      sum1(ifh,ivr,ilv,iar,iob) =
     +                    sum1(ifh,ivr,ilv,iar,iob) +
     +                    abs(dxf(i)-dxo(i))*area_factor(i)

                      sum2(ifh,ivr,ilv,iar,iob) =
     +                    sum2(ifh,ivr,ilv,iar,iob) + 
     +                    max(abs(dxf(i)),abs(dxo(i)))
     +                       *area_factor(i)

                      sum3(ifh,ivr,ilv,iar,iob) =
     +                    sum3(ifh,ivr,ilv,iar,iob) +
     +                    abs(dyf(i)-dyo(i))*area_factor(i)

                      sum4(ifh,ivr,ilv,iar,iob) =
     +                    sum4(ifh,ivr,ilv,iar,iob) +
     +                    max(abs(dyf(i)),abs(dyo(i)))
     +                       *area_factor(i)
...
90         continue
     ...
     +     sumprod(ifh,ivr,ilv,iar,iob)/weigh(ifh,ivr,ilv,iar,iob)
           ...
           sumMAE(ifh,ivr,ilv,iar,iob)=
     +     sumMAE(ifh,ivr,ilv,iar,iob)/weigh(ifh,ivr,ilv,iar,iob)

           sum1(ifh,ivr,ilv,iar,iob)=
     +     sum1(ifh,ivr,ilv,iar,iob)/weigh(ifh,ivr,ilv,iar,iob)

           sum2(ifh,ivr,ilv,iar,iob)=
     +     sum2(ifh,ivr,ilv,iar,iob)/weigh(ifh,ivr,ilv,iar,iob)

           sum3(ifh,ivr,ilv,iar,iob)=
     +     sum3(ifh,ivr,ilv,iar,iob)/weigh(ifh,ivr,ilv,iar,iob)

           sum4(ifh,ivr,ilv,iar,iob)=
     +     sum4(ifh,ivr,ilv,iar,iob)/weigh(ifh,ivr,ilv,iar,iob)
