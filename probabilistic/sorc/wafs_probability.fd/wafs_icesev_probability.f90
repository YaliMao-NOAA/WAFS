        program wafs_icesev_probability
!
...
real,       allocatable :: cat_3(:),cat_4(:),prob_mode(:),prob_90(:)
...
double precision cat3,cat4,prob90,mode
...
allocate (fgrid(maxgrd,tfiles),fgrid_im(maxgrd),fstd(tfiles),fst(tfiles),                   &
          cat_3(maxgrd),cat_4(maxgrd),prob_90(maxgrd),prob_mode(maxgrd),ens_avg(maxgrd),&
...
do ivar = 1, nvar  
  ...
  do n=1,maxgrd
    ...
    call probability(fstd,inum,cat3,cat4,prob90)
    cat_3(n)=cat3
    cat_4(n)=cat4
    prob_90(n)=prob90
    ...
    if(cat3.eq.0.0.or.cat4.eq.0.0) then
      print *, '   '
      print *,  ' Sorted Ensemble Data Example at Point',n
      write (*,'(10f8.2)') (fstd(i),i=1,inum)
      print *,  ' cat3, cat4, 90% Probability at Point',n
      write (*,'(5f12.1)') cat_3(n),cat_4(n), prob_90(n),ens_avg(n),ens_spr(n)
      print *, '   '
    endif
  ...
  gfldo%fld(1:maxgrd)=cat_3(1:maxgrd)
...
! kensout(5)=50          !: OCT 45, Spatial Smoothing of Product or Probability (if byte 44 = 23), 50=50% probability 
  ...
  gfldo%fld(1:maxgrd)=cat_4(1:maxgrd)
