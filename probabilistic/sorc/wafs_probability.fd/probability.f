...
c     print *, 'Calculates the L-moment ratios, by prob. weighted'
      ...
      nsubtotal=0
      do i=1,inum
         if(nint(fst(i))==2 .or. nint(fst(i))==5) then
            nsubtotal(1)=nsubtotal(1)+1
         endif
         if(nint(fst(i))==5) then
            nsubtotal(2)=nsubtotal(2)+1
         endif
      enddo

      fvalue10=1.0*nsubtotal(1)/inum
      fvalue90=1.0*nsubtotal(2)/inum
