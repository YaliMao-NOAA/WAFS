      subroutine probability_gtg(fst,inum,nthreshold,thresholds,fvalues)
...
C--------+---------+---------+---------+---------+----------+---------+--
      ...
      integer nthreshold
      real thresholds(nthreshold)
      real fvalues(nthreshold)
      integer :: i, j,nsubtotal(nthreshold)
...
c     print *, 'Calculates the L-moment ratios, by prob. weighted'
      ...
      do j = 1, nthreshold
         ...
         do i=1,inum
            if(fst(i) >= thresholds(j) .and. fst(i)<1.1) then
               nsubtotal(j)=nsubtotal(j)+1
            endif
         enddo

         fvalues(j)=1.0*nsubtotal(j)/inum
      enddo
