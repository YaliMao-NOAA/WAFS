...
if [[ -n $leveltype ]] ; then  # on hybrid level
  ...
  if [[ $prd == potential || $prd =~ severity || $prd == probability ]] ; then
      ...
      declare -a levels=(150 200 250 300 350 400)
      declare -a fllevels=("FL450" "FL390" "FL340" "FL300" "FL270" "FL240")
  fi
fi
if [[ -n $4 ]] ; then
  levels="$4"
...
echo     cp $HOMEsave/grads/gtgColors.gs colors.gs
else
...
if [[ $prd =~ severity ]] ; then
   ...
   cp $HOMEsave/grads/cbar.gs cbar.gs
else
...
# Get the number of elements in array1
num_elements=${#levels[@]}

# Loop through the indices
for (( i=0; i<num_elements; i++ )); do
    echo "Element from array1: ${levels[i]}, Element from array2: ${fllevels[i]}"
    lvl=${levels[i]}
...
'set string 1  tl 10'
'set strsiz .2'
'draw string 0.6 7.9 probabilistics edr>=0.20  on ${clvl}'
'set string 1  tr 1'
'set strsiz 0.15'
'draw string 10.5 7.7 $analysis at $datetime'
'printim $prefix$datetime.${clvl}.$fieldshort.0p20.png png'
EOF
