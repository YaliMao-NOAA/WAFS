...
EXECverf=${EXECverf:-/u/Binbin.Zhou/work/grid2grid/verf_g2g.v3.0.0/exec} 
...
if [ $modnam = urma ]; then
  ...
  if [ $vgrid = '184' ] ; then
    COMURMA=${COMURMA:-/com/urma/prod/urma2p5}
  fi
  if [ $vgrid = '91' ] ; then
    COMURMA=/com/rtma/prod/akrtma       #Alaska URMA not available, use RTMA instead
  fi
  ...
  for cyc in 00 03 06 09 12 15 18 21 ; do
     ...
     if [ $vgrid = '184' ] ; then   # over CONUS, use HRRR's 0hr as ceiling truth
       ...
       echo ceiling.t${cyc}z.hrrr.grib2|$EXECverf/verf_g2g_ceiling_adjust
...
if [ $modnam = conusarw ] || [ $modnam = conusnmmb ] ; then   # HIRESW data
  ...
  for cyc in 00 12 ; do 
    for fhr in 03 06 09 12 15 18 21 24 27 30 33 36 ; do 
      file=$COMHIRESW.$vday/${modnam}.t${cyc}z.awp5kmf${fhr}.grib2 
...
if [ $modnam = narremean ] ; then
  COMNARRE=${COMNARRE:-/com/rap/prod/narre}
  for cyc in 00 03 06 09 12 15 18 21 ; do
 ...
 if [ $vgrid = '184' ] ; then
  ...
  for cyc in 00 03 06 09 12 15 18 21 ; do
    for fhr in 03 06 09 12 15 ; do
      ...
      echo hrrr.${cyc}.${fhr}.ceiling|$EXECverf/verf_g2g_ceiling_adjust
...
#############################################################
#6:Get convert 16km  SREFMEAN  to 2.5km grid184 or grid91 over Alasks
#############################################################
