...
while [[ ${t} -le ${tend} ]]; do
  ...
  if [[ ${t} -ge 11 && ${t} -le 23 ]]; then  ## make sure VT1 >=0
    if [[ -e $obsvdir.${vday}/${ohead}.t${t}z.f00 ]]; then
      ...
      $USHverf_g2g/verf_g2g_prepg2g.sh < user.ctl >output_prepg2g.${vtype}${nest}
...
##  cp $savefile $COMVSDB/${RUN}/.
  ...
  if [ $SENDDBN = YES ]; then
    $DBNROOT/bin/dbn_alert MODEL VERIF_GRID2GRID $job $COMVSDB/${RUN}/$savefile 
  fi
fi
rm -f usr.ctl
